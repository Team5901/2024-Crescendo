...
public class SliderIOFalcon implements SliderIO {
  ...
  // private static final double sensorResolution =
  // (double)Constants.SliderSubsystem.sensorResolution;
  ...
  public SliderIOFalcon() {
    sliderMotor =
        new WPI_TalonFX(
            Constants.SliderSubsystem.deviceID, "rio"); // change rio to canivore device name
  ...
  public void setPosition(double positionInch, double ffVolts) {
    ...
    double setPointSensorCounts =
        Conversions.MetersToFalcon(
            Units.inchesToMeters(positionInch),
    ...
    // double setPointSensorCounts = positionInch/(sprocketCircumferenceInch)/Math.PI/gearRatio*
    // sensorResolution*10.0;
    sliderMotor.set(TalonFXControlMode.MotionMagic, setPointSensorCounts);
  }

  @Override
  public void updateState() {
    positionMotorShaftRot =
        Conversions.falconToDegrees(sliderMotor.getSelectedSensorPosition(), gearRatio) / 360.0;
  ...
  public void configurePID(double kP, double kI, double kD) {
    ...
    sliderMotor.setStatusFramePeriod(
        StatusFrameEnhanced.Status_13_Base_PIDF0, 10, Constants.SliderSubsystem.kTimeoutMs);
    sliderMotor.setStatusFramePeriod(
        StatusFrameEnhanced.Status_10_MotionMagic, 10, Constants.SliderSubsystem.kTimeoutMs);
    ...
    sliderMotor.configSupplyCurrentLimit(
        new SupplyCurrentLimitConfiguration(
            true,
            Constants.SliderSubsystem.maxCurrentAmps,
            Constants.SliderSubsystem.maxCurrentAmps + 5.0,
            1.0));
    // sliderMotor.configSupplyCurrentLimit(new
    // SupplyCurrentLimitConfiguration(true,Constants.SliderSubsystem.maxCurrentAmps/2.0,20,0.5));
    ...
    double CruiseVel =
        Conversions.MPSToFalcon(
            Units.inchesToMeters(Constants.SliderSubsystem.maxLinearVelocityInchPerSec),
            Units.inchesToMeters(sprocketCircumferenceInch),
            gearRatio);
    double CruiseAcc =
        Conversions.MPSToFalcon(
            Units.inchesToMeters(Constants.SliderSubsystem.maxLinearAccelerationInchPerSec),
            Units.inchesToMeters(sprocketCircumferenceInch),
            gearRatio);
    ...
    sliderMotor.configAllowableClosedloopError(
        0, Constants.SliderSubsystem.allowableSmartMotionPosErrorRotations * 2048);
